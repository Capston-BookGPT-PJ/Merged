...
public class BrowseActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ItemDecoration 추가
        usersRecyclerView.addItemDecoration(new RecyclerView.ItemDecoration() {
            @Override
            public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
                int position = parent.getChildAdapterPosition(view); // item position
                int column = position % 2; // spanCount = 2

                // 좌우 간격 계산
                outRect.left = spacingPx - column * spacingPx / 2;
                outRect.right = (column + 1) * spacingPx / 2;

                // 상하 간격
                if (position < 2) { // 첫 줄 위쪽
        ...
        popularBooks.setOnClickListener(v -> {
            findViewById(R.id.booksAndReviewsLayout).setVisibility(View.VISIBLE);
            findViewById(R.id.hashtagsLayout).setVisibility(View.GONE);
            findViewById(R.id.usersLayout).setVisibility(View.GONE);
        });

        popularTags.setOnClickListener(v -> {
            findViewById(R.id.booksAndReviewsLayout).setVisibility(View.GONE);
